...
export const MAIN_FABRIQ_DEGEN_FEE_ROTATION_V1 = Object.freeze({
  ...
  entry: {
    condition: "Deploy SOL-only into a dynamic fee-density bid_ask range only after Fabriq/Degen-style discovery passes Meridian liquidity, fee-velocity, volume/active-TVL, holder, organic, cooldown, and rug checks.",
    single_side: "sol",
  ...
  range: {
    type: "dynamic_fee_density_recovery_hold",
    bins_below: 35,
    bins_below_min: 12,
    bins_below_max: 120,
    bins_above: 0,
    dynamic_range_width_enabled: true,
    dynamic_pool_sizing_enabled: true,
    ohlcv_entry_gate_enabled: true,
    notes: "Live dynamic range width targets tighter downside for higher-mcap fee-dense pools and wider downside for lower-mcap/thinner pools. Runtime deploy size is capped to active TVL share and entry requires DexPaprika/GMGN/OKX OHLCV evidence.",
  },
  exit: {
    take_profit_pct: 2,
    stop_loss_pct: -3.5,
