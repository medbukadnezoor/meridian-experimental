...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  if (source === "gmgn") {
    ...
    // Apply blacklist/blocklist to GMGN-sourced pools in the merged universe
    if (discovery.pools) {
      const before = discovery.pools.length;
      discovery.pools = discovery.pools.filter((p) => {
        if (isBlacklisted(p.base?.mint)) {
          log("blacklist", `[both] Filtered blacklisted token ${p.base?.symbol} (${p.base?.mint?.slice(0, 8)}) in pool ${p.name}`);
          return false;
        }
        if (p.dev && isDevBlocked(p.dev)) {
          log("dev_blocklist", `[both] Filtered blocked deployer ${p.dev?.slice(0, 8)} token ${p.base?.symbol}`);
          return false;
        }
        return true;
      });
      if (discovery.pools.length < before) log("blacklist", `[both] Filtered ${before - discovery.pools.length} blacklisted/blocked pool(s)`);
    }
  } else {
