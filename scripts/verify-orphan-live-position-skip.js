...
// Isolate the PnL poll loop body: from the poll's `for (const p of result.positions) {`
// up to the first exit evaluation call.
const pollLoopStart = src.indexOf("for (const p of result.positions) {");
check("PnL poll loop over result.positions exists", pollLoopStart !== -1);

const firstExitCallIdx = src.indexOf("updatePnlAndCheckExits(p.position, p, config.management)", pollLoopStart);
check("PnL poll still evaluates exits (updatePnlAndCheckExits present)", firstExitCallIdx !== -1);
...
check(
  ...
  /log\(\s*["']cron_error["']/.test(pollPreamble) &&
    /Untracked live position skipped/.test(pollPreamble),
);
check(
  "PnL poll orphan Telegram alert gated on telegramEnabled() and reuses sendMessage",
  /if\s*\(\s*telegramEnabled\(\)\s*\)\s*\{[\s\S]*?sendMessage\(/.test(pollPreamble),
);
check(
  "PnL poll orphan dedup via _alertedOrphans set",
  /_alertedOrphans\.has\(p\.position\)/.test(pollPreamble) &&
    /_alertedOrphans\.add\(p\.position\)/.test(pollPreamble),
);
...
if (mgmtFilterIdx !== -1) {
  const mgmtBlock = src.slice(mgmtFilterIdx, mgmtFilterIdx + 800);
  check(
    "Management filter keeps tracked positions (getTrackedPosition guard)",
    /getTrackedPosition\(p\.position\)/.test(mgmtBlock),
  );
  check(
