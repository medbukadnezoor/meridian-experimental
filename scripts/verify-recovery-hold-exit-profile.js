...
import { allowsOutOfRangeExit } from "../oor-exit-policy.js";
...
async function main() {
  ...
  try {
    ...
    assert.equal(
      allowsOutOfRangeExit(0.2, recoveryConfig(), { rangeSide: "above_range", oorStage: "hard" }),
      true,
      "hard above-range OOR can close at non-negative PnL below ordinary non-fee floor",
    );
    assert.equal(
      allowsOutOfRangeExit(0.2, recoveryConfig(), { rangeSide: "below_range", oorStage: "hard" }),
      false,
      "hard below-range OOR below ordinary non-fee floor should hold",
    );

    track("oor-above-hard-positive");
    setTrackedFields(tempDir, "oor-above-hard-positive", { out_of_range_since: new Date(Date.now() - 130 * 60_000).toISOString() });
    const oorAboveHardPositive = updatePnlAndCheckExits(
      "oor-above-hard-positive",
      makePosition("oor-above-hard-positive", {
        pnl_pct: 0.01,
        in_range: false,
    ...
    assert.equal(oorAboveHardPositive?.action, "OUT_OF_RANGE", "hard above-range OOR may close at non-negative PnL");
    assert.equal(oorAboveHardPositive?.urgent, true, "hard above-range OOR should be urgent");
    assert.equal(oorAboveHardPositive?.indicatorPolicy, "bypass", "hard above-range OOR should bypass indicators");

    track("oor-below-hard-positive");
    setTrackedFields(tempDir, "oor-below-hard-positive", { out_of_range_since: new Date(Date.now() - 130 * 60_000).toISOString() });
    const oorBelowHardPositive = updatePnlAndCheckExits(
      "oor-below-hard-positive",
      makePosition("oor-below-hard-positive", {
        pnl_pct: 0.01,
        in_range: false,
    ...
    assert.equal(oorBelowHardPositive, null, "hard below-range OOR below ordinary non-fee floor should hold");
