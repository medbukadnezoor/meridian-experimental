...
function runtimeConfig(overrides = {}) {
  return {
    screening: {
      ...
      criticalThinMcapUsd: 275_000,
      criticalThinActiveTvlUsd: 5_000,
      criticalThinWatchMcapUsd: 450_000,
...
const mcapCriticalMissingProof = evaluateCriticalThinEntryOverlay(
  { mcap: 274_999, active_tvl: 20_000 },
  runtimeConfig(),
  { fabriqOhlcvEntryGate: acceptedGate(), dynamicPoolSizingDecision: sizingDecision({ active_tvl_usd: 20_000 }) },
);
assert.strictEqual(mcapCriticalMissingProof.ok, false, "mcap below 275k blocks when fee/activity proof is missing");
assert.strictEqual(mcapCriticalMissingProof.overlay.bucket, "critical", "mcap below 275k is critical");
...
assert.strictEqual(built.screening.criticalThinMcapUsd, 275_000, "config builder maps mcap threshold");
...
assert.strictEqual(example.dynamicRangeWidthEnabled, true, "example config enables dynamic range width");
assert.strictEqual(example.dynamicRangeWidthMode, "live", "example config sets dynamic range width live");
...
console.log(JSON.stringify({
  ...
  checks: [
    "mcap below 275k blocks without proof",
    ...
    "dynamic range width remains live in example config",
  ],
}, null, 2));
