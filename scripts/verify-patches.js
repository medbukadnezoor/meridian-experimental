...
const OOR_ABOVE_HARD_CLOSE_POLICY_VERIFIER_PATH = join(__dirname, "verify-oor-above-hard-close-policy.js");
...
function runOorAboveHardClosePolicyProof() {
  return runJsonVerifier(OOR_ABOVE_HARD_CLOSE_POLICY_VERIFIER_PATH, "verify-oor-above-hard-close-policy");
}
...
function buildChecks() {
  ...
  const oorAboveHardClosePolicyProof = runOorAboveHardClosePolicyProof();
  ...
  return [
    ...
    },
    {
      ...
      test: () =>
        ...
        dynamicRangeWidthGuardProof?.checks?.includes("coverage math matches 16/20/30 percent target fixtures") &&
        dynamicRangeWidthGuardProof?.checks?.includes("low-mcap pools keep wider downside while high-mcap fee-dense pools tighten below 35 bins") &&
        dynamicRangeWidthGuardProof?.checks?.includes("dynamic sizing final share overrides stale candidate deploy share") &&
        ...
        dynamicRangeWidthGuardProof?.samples?.high_mcap_tight?.decision === "override" &&
        Number(dynamicRangeWidthGuardProof?.samples?.high_mcap_tight?.final_bins_below) === 16 &&
    ...
    },
    {
      ...
      test: () =>
        ...
        criticalThinEntryOverlayProof?.checks?.includes("mcap below 275k blocks without proof") &&
        ...
        criticalThinEntryOverlayProof?.checks?.includes("dynamic range width remains live in example config") &&
    ...
    {
      file: "scripts/verify-oor-above-hard-close-policy.js",
      label: "[Main OOR above-range hard close] hard above-range OOR escapes recovery floor while negative/below-range cases stay protected",
      test: () =>
        oorAboveHardClosePolicyProof?.success === true &&
        oorAboveHardClosePolicyProof?.checks?.includes("hard above-range OOR bypasses recovery floor at non-negative PnL") &&
        oorAboveHardClosePolicyProof?.checks?.includes("soft above-range OOR still respects recovery floor") &&
        oorAboveHardClosePolicyProof?.checks?.includes("hard above-range OOR does not close negative PnL") &&
        oorAboveHardClosePolicyProof?.checks?.includes("hard below-range OOR still respects recovery floor") &&
        oorAboveHardClosePolicyProof?.cases?.aboveHard === "OUT_OF_RANGE" &&
        oorAboveHardClosePolicyProof?.cases?.aboveHardStage === "hard" &&
        oorAboveHardClosePolicyProof?.cases?.aboveHardUrgent === true &&
        oorAboveHardClosePolicyProof?.cases?.aboveSoft === null &&
        oorAboveHardClosePolicyProof?.cases?.aboveHardNegative === null &&
        oorAboveHardClosePolicyProof?.cases?.belowHard === null,
    },
    {
    ...
    {
      file: "config-builder.js",
      label: "[OHLCV drawdown shadow] shadow observation config maps into runtime config",
      test: (src) =>
        src.includes("ohlcvDrawdownShadowEnabled: u.ohlcvDrawdownShadowEnabled ?? true") &&
    ...
    {
      file: "tools/executor.js",
      label: "[OHLCV drawdown shadow] update_config maps shadow observation fields only",
      test: (src) =>
    ...
    {
      file: "state.js",
      label: "[OHLCV drawdown shadow] state helper dedupes first trigger per position/rule without returning an exit",
      test: (src) =>
