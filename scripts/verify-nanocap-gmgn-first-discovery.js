#!/usr/bin/env node
/**
 * Synthetic/source proof for PHASE2A-NANOCAP-GMGN-FIRST-DISCOVERY-T1.
 *
...
import { buildConfig, resolveConfigFromPath } from "../config-builder.js";
...
assert.strictEqual(gmgnConfig.gmgn.minMcap, 30_000, "GMGN min mcap inherits nanocap mcap floor");
assert.strictEqual(gmgnConfig.gmgn.maxMcap, 600_000, "GMGN max mcap inherits nanocap mcap ceiling");
assert.strictEqual(gmgnConfig.gmgn.minTvl, 10_000, "GMGN min TVL inherits nanocap TVL floor");
assert.strictEqual(gmgnConfig.gmgn.minVolume, 1000, "GMGN min volume inherits nanocap volume floor");
assert.strictEqual(gmgnConfig.gmgn.minHolders, 100, "GMGN min holders inherits nanocap holder floor");
...
const exampleProof = resolveConfigFromPath(path.join(ROOT, "user-config.example.json"), {
...
const deterministicGate = indexOfAfterOrThrow(screeningSource, "const vetoReason = getDeterministicCandidateVetoReason", discoveryBranch, "deterministic nanocap veto");
...
for (const [label, index] of [
  ...
  ["deterministic nanocap veto", deterministicGate],
...
console.log(JSON.stringify({
  ...
  nanocapExample: {
    screeningSource: exampleProof.config.screening.source,
    gmgnKeyIsEnvReferenced: exampleProof.config.gmgn.apiKey === "synthetic-gmgn-key",
    gmgnEnrichLimit: exampleProof.config.gmgn.enrichLimit,
    gmgnRequestDelayMs: exampleProof.config.gmgn.requestDelayMs,
  },
  sourceSafety: {
