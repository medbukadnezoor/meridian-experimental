...
check("telegram registers native slash command popup menu", () =>
  has(files.telegram, /export\s+async\s+function\s+setCommandMenu\(commands\s*=\s*\[\]\)/) &&
  has(files.telegram, /postTelegram\("setMyCommands"[\s\S]*?includeChatId:\s*false/) &&
  has(files.index, /const\s+TELEGRAM_SLASH_COMMANDS\s*=\s*\[/) &&
  has(files.index, /async\s+function\s+registerTelegramSlashCommands\(\)/) &&
  has(files.index, /setCommandMenu\(TELEGRAM_SLASH_COMMANDS\.map/) &&
  has(files.index, /registerTelegramSlashCommands\(\)\.catch/)
);

check("telegram native menu commands normalize bot username suffix before routing", () =>
  has(files.index, /function\s+normalizeTelegramCommandText\(rawText\)/) &&
  has(files.index, /\^\\\/\(\[A-Za-z0-9_\]\+\)\(\?:@\[A-Za-z0-9_\]\+\)\?\(\?=\\s\|\$\)/) &&
  has(files.index, /String\(command\)\.toLowerCase\(\)/) &&
  has(files.index, /const\s+text\s*=\s*normalizeTelegramCommandText\(msg\?\.callbackData\s*\|\|\s*msg\?\.text\s*\|\|\s*""\)/)
);
