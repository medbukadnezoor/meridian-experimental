...
export function buildCandidateDecisionContext(candidate = {}) {
  ...
  return {
    ...
    feePct: safeNumber(candidate.fee_pct ?? candidate.base_fee),
    feeVelocityUsdPerMin: safeNumber(candidate.fee_velocity_usd_per_min),
    volumeActiveTvlMultiple: safeNumber(candidate.volume_active_tvl_multiple),
    feeLane: candidate.fee_lane ?? candidate._feeLane ?? candidate.two_lane_classification?.lane ?? null,
    looseLaneQualified: candidate.loose_lane_qualified ?? candidate.two_lane_classification?.looseLaneQualified ?? null,
    looseLaneVeto: candidate.loose_lane_veto ?? candidate.two_lane_classification?.looseLaneVeto ?? null,
    twoLaneClassification: candidate.two_lane_classification ?? null,
    momentumScoreV1: safeNumber(candidate.momentum_score_v1 ?? candidate.momentum_score?.primary?.momentum_score_v1),
    ...
    momentumConfidence: safeNumber(candidate.momentum_confidence ?? candidate.momentum_score?.primary?.confidence),
    ...
    dynamicEntryShadow: candidate.dynamic_entry_shadow ?? candidate.momentum_score?.primary?.dynamic_entry_shadow ?? null,
    dynamicEntryLabel: candidate.dynamic_entry_shadow?.entry_label ?? candidate.momentum_score?.primary?.dynamic_entry_shadow?.entry_label ?? null,
    dynamicEntryScore: safeNumber(candidate.dynamic_entry_shadow?.dynamic_score ?? candidate.momentum_score?.primary?.dynamic_entry_shadow?.dynamic_score),
