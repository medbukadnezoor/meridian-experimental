...
function getRecoveryHoldNonFeeExitMinNetPnlPct(mgmtConfig = {}) {
  if (!mgmtConfig.recoveryHoldProfileEnabled) return null;
  return toFiniteNumberOrNull(mgmtConfig.recoveryHoldNonFeeExitMinNetPnlPct) ?? 0;
}

function allowsRecoveryHoldNonFeeExit(currentPnlPct, mgmtConfig = {}, requirePositive = false) {
  if (!requirePositive) return true;
  const minNetPnlPct = getRecoveryHoldNonFeeExitMinNetPnlPct(mgmtConfig);
...
export function updatePnlAndCheckExits(position_address, positionData, mgmtConfig) {
  ...
  const canOutOfRangeClose = allowsRecoveryHoldNonFeeExit(currentPnlPct, mgmtConfig, mgmtConfig.requirePositivePnlForOutOfRangeExit);
